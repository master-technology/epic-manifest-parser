...
// based on https://github.com/Sprayxe/unreal.js/blob/29f04689060a0fcbe8d5c0f25f1f2a168c8662b2/src/ue4/reader/FArchive.ts
export default class FArchive {
  ...
  readString(): string {
    ...
    if (length < -65536 || length > 65536) throw new Error(`Invalid String length '${length}'`)
    if (length < 0) {
      ...
      return Buffer.from(dat).toString("utf-8")
    } else {
      if (length === 0) return ""
  ...
  readFString(): FString {
    return new FString(this.readString())
  }

  readArray(fn: (ar: FArchive) => any) {
    let arrLength = this.readUInt32();
    let dat = [];
    for (let i = 0; i < arrLength; ++i) dat.push(fn(this))
    return dat;
  }

  readEnum(Enum, value = this.readUInt8()) {
    return Enum[Enum[value]];
  }

  toString() {
    return `Pos: ${this.tell()}, Size: ${this.size}`
  }
}